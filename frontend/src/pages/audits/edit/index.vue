          ...
          <q-list class="home-drawer">
            <q-item style="padding: 0px">
              ...
              <template v-if="$settings.reviews.enabled">
                ...
                >
                  <q-btn
                    ...
                    dense
                    color="secondary"
                    :label="$t('btn.topButtonSection.submitReview')"
                    no-caps
                  ...
                  >
                    <q-tooltip
                      ...
                      >{{
                        $t("tooltip.topButtonSection.submitReview")
                      }}</q-tooltip
                    >
                  </q-btn>
                </q-item-section>
                <q-item-section
                  ...
                  v-if="
                    [
                      AUDIT_VIEW_STATE.REVIEW_EDITOR,
                      AUDIT_VIEW_STATE.REVIEW_ADMIN,
                      AUDIT_VIEW_STATE.REVIEW_ADMIN_APPROVED,
                    ].includes(frontEndAuditState)
                  "
                >
                  <q-btn
                    class="q-mx-xs q-px-xs"
                    ...
                    dense
                    color="amber-9"
                    :label="$t('btn.topButtonSection.cancelReview')"
                    no-caps
                    @click="toggleAskReview"
                  >
                    <q-tooltip
                      ...
                      content-class="text-bold"
                      >{{
                        $t("tooltip.topButtonSection.cancelReview")
                      }}</q-tooltip
                    >
                  </q-btn>
                </q-item-section>
                ...
                >
                  <q-btn
                    ...
                    dense
                    color="green"
                    :label="$t('btn.topButtonSection.approve')"
                    no-caps
                  ...
                  >
                    <q-tooltip
                      ...
                      :delay="500"
                      content-class="text-bold"
                      >{{ $t("tooltip.topButtonSection.approve") }}</q-tooltip
                    >
                  </q-btn>
                </q-item-section>
                <q-item-section
                  ...
                  v-if="
                    [
                      AUDIT_VIEW_STATE.REVIEW_APPROVED,
                      AUDIT_VIEW_STATE.REVIEW_ADMIN_APPROVED,
                      AUDIT_VIEW_STATE.APPROVED_APPROVED,
                    ].includes(frontEndAuditState)
                  "
                >
                  <q-btn
                    ...
                    dense
                    color="warning"
                    :label="$t('btn.topButtonSection.removeApproval')"
                    no-caps
                    @click="toggleApproval"
                  >
                    <q-tooltip
                      ...
                      content-class="text-bold"
                      >{{
                        $t("tooltip.topButtonSection.removeApproval")
                      }}</q-tooltip
                    >
                  </q-btn>
                </q-item-section>
              ...
              <q-item-section side class="topButtonSection">
                <!-- <q-btn flat color="info" @click="openSelect">
                  <q-tooltip
                    anchor="bottom middle"
                    self="center left"
                    :delay="500"
                    content-class="text-bold"
                    >{{ $t("tooltip.downloadReport") }}</q-tooltip
                  >
                ...
                <q-select
                  ...
                  class="download-report-opt"
                  dropdown-icon="img:/down-arrow.png"
                  :options="options"
                  dense
                  options-dense
            ...
            <q-item
              v-if="
                !currentAuditType ||
                !currentAuditType.hidden.includes('network')
              "
            ...
            <div
              v-if="
                !currentAuditType ||
                !currentAuditType.hidden.includes('findings')
              "
            >
              ...
              >
                  ...
                  <q-item-section avatar>
                    ...
                    >
                      <q-tooltip
                        ...
                        :delay="500"
                        content-class="text-bold"
                        >{{ $t("tooltip.sortOptions") }}</q-tooltip
                      >
                      <q-menu
                        ...
                        <q-item>
                          <q-item-section>
                            <q-option-group
                              ...
                              :options="
                                getSortOptions(
                                  categoryFindings.sortOption.category
                                )
                              "
                              type="radio"
                        ...
                        <q-item>
                          <q-item-section>
                            <q-btn
                              ...
                              :disable="!categoryFindings.sortOption.sortAuto"
                              :color="
                                categoryFindings.sortOption.sortOrder === 'asc'
                                  ? 'green'
                                  : ''
                              "
                              @click="
                        ...
                        <q-item>
                          <q-item-section>
                            <q-btn
                              ...
                              :disable="!categoryFindings.sortOption.sortAuto"
                              :color="
                                categoryFindings.sortOption.sortOrder === 'desc'
                                  ? 'green'
                                  : ''
                              "
                              @click="
                ...
                <q-list no-border>
                  <draggable
                    ...
                    @end="
                      moveFindingPosition($event, categoryFindings.category)
                    "
                    handle=".handle"
                    ...
                    >
                      ...
                      >
                        <q-item-section
                          side
                          v-if="
                            !categoryFindings.sortOption.sortAuto &&
                            frontEndAuditState === AUDIT_VIEW_STATE.EDIT
                          "
                        >
                          <q-icon
      ...
      handler(newVal, oldVal) {
        var result = _.chain(this.audit.findings)
          ...
          .map((value, key) => {
            ...
            var sortOption = this.audit.sortFindings.find(
            ...
            if (!sortOption) {
              ...
              if (sortOption)
                // found sort option from vuln categories
  ...
  methods: {
    ...
    getFindingSeverity: function (finding) {
      ...
      if (cvss.success) {
        ...
        let sortOption = this.audit.sortFindings.find(
    ...
    getMenuSection: function () {
      if (
        ...
        this.$router.currentRoute.name === "general"
      )
        return { menu: "general", room: this.auditId };
      else if (
        this.$router.currentRoute.name &&
        this.$router.currentRoute.name === "network"
      )
        return { menu: "network", room: this.auditId };
      else if (
        this.$router.currentRoute.name &&
        this.$router.currentRoute.name === "addFindings"
      )
        return { menu: "addFindings", room: this.auditId };
      else if (
        this.$router.currentRoute.name &&
        this.$router.currentRoute.name === "editFinding" &&
        this.$router.currentRoute.params.findingId
      )
        return {
          menu: "editFinding",
          finding: this.$router.currentRoute.params.findingId,
          room: this.auditId,
        };
      else if (
        this.$router.currentRoute.name &&
        this.$router.currentRoute.name === "editSection" &&
        this.$router.currentRoute.params.sectionId
      )
        return {
          menu: "editSection",
          section: this.$router.currentRoute.params.sectionId,
          room: this.auditId,
        };
    ...
    isUserAReviewer: function () {
      ...
      var isCollaborator = this.audit.collaborators.some(
        (element) => element._id === UserService.user.id
      );
      var isReviewer = this.audit.reviewers.some(
        (element) => element._id === UserService.user.id
      );
    ...
    userHasAlreadyApproved: function () {
      return this.audit.approvals.some(
        (element) => element._id === UserService.user.id
      );
    },
    ...
    getUIState: function () {
      if (!this.$settings.reviews.enabled || this.audit.state === "EDIT") {
        this.frontEndAuditState = this.isUserAnEditor()
          ? Utils.AUDIT_VIEW_STATE.EDIT
          : Utils.AUDIT_VIEW_STATE.EDIT_READONLY;
      } else if (this.audit.state === "REVIEW") {
        if (!this.isUserAReviewer()) {
          this.frontEndAuditState = this.isUserAnEditor()
            ? Utils.AUDIT_VIEW_STATE.REVIEW_EDITOR
            : Utils.AUDIT_VIEW_STATE.REVIEW_READONLY;
          return;
        }
        if (this.isUserAnEditor()) {
          this.frontEndAuditState = this.userHasAlreadyApproved()
            ? Utils.AUDIT_VIEW_STATE.REVIEW_ADMIN_APPROVED
            : Utils.AUDIT_VIEW_STATE.REVIEW_ADMIN;
          return;
        }
        this.frontEndAuditState = this.userHasAlreadyApproved()
          ? Utils.AUDIT_VIEW_STATE.REVIEW_APPROVED
          : Utils.AUDIT_VIEW_STATE.REVIEW;
      } else if (this.audit.state === "APPROVED") {
        if (!this.isUserAReviewer()) {
          this.frontEndAuditState = Utils.AUDIT_VIEW_STATE.APPROVED_READONLY;
        } else {
          this.frontEndAuditState = this.userHasAlreadyApproved()
            ? Utils.AUDIT_VIEW_STATE.APPROVED_APPROVED
            : Utils.AUDIT_VIEW_STATE.APPROVED;
        }
      }
    },
    ...
    updateSortFindings: function () {
      AuditService.updateAuditSortFindings(this.auditId, {
        sortFindings: this.audit.sortFindings,
      });
    },
    ...
    moveFindingPosition: function (event, category) {
      var index = this.audit.findings.findIndex((e) => {
        if (category === "No Category") return !e.category;
        else return e.category === category;
      });
      if (index > -1) {
        var realOldIndex = event.oldIndex + index;
        var realNewIndex = event.newIndex + index;

        AuditService.updateAuditFindingPosition(this.auditId, {
    ...
    toggleAskReview: function () {
      AuditService.updateReadyForReview(this.auditId, {
        state: this.audit.state === "EDIT" ? "REVIEW" : "EDIT",
      })
        .then(() => {
